...
Define org and melpa as package sources, and install /use-package/ if
it's not already there. Always ensure packages being loaded are there
(or else it'll automatically download from melpa).
...
#+BEGIN_SRC emacs-lisp
  (use-package cl-lib)
  ;;(require 'cl) No longer required; replaced by cl-lib
  (require 'generic-x)
  (require 'smallurl)
#+END_SRC
...
Other themes I sometimes use: /color-theme/, /monokai-theme/, and /tangotango-theme/.
...
Enable [[https://www.gnu.org/savannah-checkouts/gnu/emacs/manual/html_node/emacs/Saving-Emacs-Sessions.html][desktop save mode]]:

#+BEGIN_SRC emacs-lisp
  (desktop-save-mode 1)
  (add-to-list 'desktop-path (cjp-emacs-structure-dir "data/desktop"))
#+END_SRC
