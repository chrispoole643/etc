...
#+BEGIN_SRC emacs-lisp
  (use-package web-mode
    :mode ("\\.html?\\'" "\\.phtml\\'" "\\.hbs\\'" "\\.tpl\\.php\\'" "\\.[agj]sp\\'"
...
nvAlt type functionaliy of having a directory of notes (zettels) that
can quickly be searched.
...
#+BEGIN_SRC emacs-lisp
  (use-package deft
    :config
    (setq deft-directory "~/Box Sync/Zettelkasten/"
          deft-recursive t
...
Trying out =org-roam= to create a Zettelkasten system, along with Deft
(previous section).

The keymapping is designed to work hand in hand with the projectile
mapping I use:

f: projectile find file
g: grep in project
i: insert org link
t: goto today's zettel
p: goto project
r: roam
z: open zettel
l: ace-link-org
...
#+BEGIN_SRC emacs-lisp
  (use-package org-roam
    ...
    ("s-r" . org-roam)
    ("s-t" . org-roam-today)
    ("s-z" . org-roam-find-file)
    ("s-i" . org-roam-insert))
#+END_SRC

*** Ace-link

Quickly open a link that's shown in the current buffer. If only one
link (http etc.) is visible, open it. If not, select from the usual
method of first letter of word...

#+BEGIN_SRC emacs-lisp
  (use-package ace-link
    :config (ace-link-setup-default)
    :bind (("s-l" . ace-link-org)))
#+END_SRC
...
#+BEGIN_SRC emacs-lisp
    (use-package org-plus-contrib
      ...
      ;; Brings back the easy templating system (i.e., `<q[tab]`) that
      ;; was changed from Org
      ;; 9.2. See https://emacs.stackexchange.com/a/46992
...
#+begin_src emacs-lisp
  (use-package org-ref
    :config
    (setq reftex-default-bibliography '((concat deft-directory "references.bib"))
          org-ref-bibliography-notes (concat deft-directory "bib-notes.org")
          org-ref-default-bibliography reftex-default-bibliography
          org-ref-pdf-directory (concat deft-directory "bibtex-pdfs/")))
#+end_src
...
This is kept as-is, but only works with Org mode prior to 9.1...
...
#+BEGIN_SRC emacs-lisp
  ...
  ;; Set my password in a private file: `grip-github-user' and
...
Allow for git info to be shown in dired buffers
...
Simple Postman-like REST client for driving APIs. Lets you build up a
literate programming type of model by having one document that holds
all these calls. Only change from defaults: don't change window focus
to the output window when submitting a command:
